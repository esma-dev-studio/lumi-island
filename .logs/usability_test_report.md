# ユーザビリティテスト報告

> **このファイルは `node tools/generate_reports.mjs` による自動生成です。手で編集しないでください**(編集内容は次回の生成で失われます)。数値の出どころは下の「入力」に挙げたJSONだけで、このファイルに手書きの数値はありません。

| 項目 | 値 |
|---|---|
| Run ID(ブラックボックスUXボット) | `ux-20260804T1357-b8ce8950` — 導出(mtime+内容ハッシュ) |
| Run ID(人間ブラインドテスト) | NOT TESTED(入力なし) |
| gitコミット | `9a511c8`(**未コミットの変更あり**) |
| 生成日時 | 2026-08-04 13:58 JST |
| 生成コマンド | `node tools/generate_reports.mjs` |
| 入力 | `.logs/ux_result.json`(更新 2026-08-04 13:57 JST) / `.logs/human_test_result.json` = **なし** |
| 過去走行の記録 | `.logs/history/runs.md`(現行のこのファイルには**今回の実測値だけ**を載せる) |

> **絶対条件の確認**: 自動テスト(UXボット)は「画面に出ている情報だけで進行できるか」を機械的に見るもので、
> **子ども向けUXが良いことの証明にはならない**。子どもが一人で遊べるかどうかは、
> 下の「2. 人間ブラインドテスト」だけが答えられる。UXボットのPASSをもって人間テストの代わりにはしない。

## 1. UXボット結果(自動)

**判定: PASS** — result=ok・意味矛盾0件・停滞0回・報告中の再釣り0回・店の誤開き0回

読み取れるのは画面のDOM(「いまやること」の文・進捗と距離・画面端の矢印・NPCマーカー・ヒント)だけで、
内部状態・座標・目標ID・所持品・デバッグAPIは一切読まない。操作も通常のキーとクリックのみ。

| 項目 | 実測値 |
|---|---|
| 走行の結末 | ok |
| 総所要時間 | 7分25秒(445秒) |
| UX判定(uxVerdict) | PASS |
| 到達フラグ | 10 / 10 |
| 表示の意味矛盾 | 0件 |
| 停滞(同じ目標で長時間動けない) | 0回 |
| 報告中の再釣り | 0回 |
| 店の意図しない開き | 0回 |
| コンソール/ページエラー | 0件 |
| カテゴリ表に無いヒント文言 | 0件 |

### 到達マイルストーン

| 経過 | 到達 |
|---|---|
| 11秒 | (1) 最初の依頼を受注できた |
| 135秒 | (2) 木材5つを集めきった |
| 157秒 | (3) ツムギへの報告が終わった |
| 177秒 | (4) つぎの依頼を受注できた |
| 208秒 | (5) クラフトの素材がそろった |
| 208秒 | (6) 初クラフトができた |
| 445秒 | (7) 家具を島に置けた(ランタンを おいた) |
| 445秒 | (8) 配置後に目標が進んだ(配置を確認) |

### 表示の意味矛盾(目的とヒントが食い違った箇所)

0件。

### 停滞

0件。

### 走行中の主なできごと

| 経過 | できごと |
|---|---|
| 6秒 | タイトル表示 |
| 7秒 | ゲーム開始 |
| 10秒 | 最初の会話が開いた |
| 21秒 | 目的地に着いたのに採取のヒントが出ない → 渦巻きで探す(もくざいを あつめよう / 0 / 5) |
| 54秒 | 停滞: もくざいを あつめよう / 1 / 5　→ 20m / ヒント: → 逆まわりで回りこむ |
| 83秒 | 停滞: もくざいを あつめよう / 1 / 5　→ 13m / ヒント: → 逆まわりで回りこむ |
| 103秒 | 目的地に着いたのに採取のヒントが出ない → 渦巻きで探す(もくざいを あつめよう / 1 / 5) |
| 130秒 | 目的地に着いたのに採取のヒントが出ない → 渦巻きで探す(もくざいを あつめよう / 3 / 5) |
| 198秒 | 目的地に着いたのに採取のヒントが出ない → 渦巻きで探す(もくざいを あつめよう / 1 / 2) |
| 204秒 | 目的地に着いたのに採取のヒントが出ない → 渦巻きで探す(いしを あつめよう / 0 / 1) |
| 208秒 | クラフト操作: カマ → ok |
| 221秒 | クラフト操作: ツリザオ → ok |
| 321秒 | 目的地に着いたのに採取のヒントが出ない → 渦巻きで探す(高台で こうせきをほろう / 1 / 3) |
| 338秒 | 目的地に着いたのに採取のヒントが出ない → 渦巻きで探す(高台で こうせきをほろう / 2 / 3) |
| 365秒 | 店のヒントが出たがEは押さない(目的: ツムギの はなしを聞こう) |
| 374秒 | 目的地に着いたのに採取のヒントが出ない → 渦巻きで探す(ヒカリゴケを あつめよう / 0 / 2) |
| 406秒 | 停滞: ヒカリゴケを あつめよう / 1 / 2　→ 14m / ヒント: → 逆まわりで回りこむ |
| 435秒 | 停滞: ヒカリゴケを あつめよう / 1 / 2　→ 10m / ヒント: → 逆まわりで回りこむ |
| 438秒 | 目的地に着いたのに採取のヒントが出ない → 渦巻きで探す(ヒカリゴケを あつめよう / 1 / 2) |
| 441秒 | クラフト操作: ランタン → ok |
| 443秒 | もちもの操作: おく → ランタン |

## 2. 人間ブラインドテスト

**判定: NOT TESTED** — `human_test_result.json` が無い(**未実施**)

`.logs/human_test_result.json` が生成されるまで、この欄が PASS になることはない。
**未実施をPASSともFAILとも書かない**(NOT TESTED のまま据え置く)。

実施したときに記録するJSONの形:

```json
{
  "runId": "hm-YYYYMMDDThhmm-xxxxxxxx",
  "date": "YYYY-MM-DD",
  "testers": [
    { "id": "A", "profile": "小学校低学年", "pass": true, "questAcceptSec": 0, "stuckCount": 0, "reachedBloom": true, "notes": "" }
  ]
}
```

### 実施手順(未実施・準備のみ)

1. 最低3名(小学生相当1名・ゲームに慣れていない成人1名・開発を知らない成人1名)。
2. README・口頭説明なしで、新規ゲームから開始してもらう。
3. 計測: 最初のNPCに話しかけるまで / 最初の依頼受注まで / 最初の採取まで / 最初のクラフトまで /
   NPC不在時に取った行動 / 釣り依頼を完了できたか / 迷った回数 / 操作説明を求めた回数 /
   ルミの木の開花まで到達できたか / 楽しかった場面 / 分かりにくかった場面。
   睡眠の理解可否は**観察項目**(合否には使わない)。
4. 受入基準: 3名中3名が最初の依頼を2分以内に受注 / 3名中3名が進行不能にならない /
   3名中2名以上が開花まで到達 / 同じ場所で2名以上が迷ったらUIかレベルデザインを直す。

## 3. 参考: 決定的リグレッションボット

判定 PASS — 完走=はい・依頼5/5・エラー0。
これは回帰保証であり、UXの証明として扱わない。詳細: `.logs/playtest_report.md`

