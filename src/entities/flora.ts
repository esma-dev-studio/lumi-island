...
// ---- 散布デコ(草・花・小石): thin instance でドローコール3つ ----
export function scatterDeco(scene: Scene): void {
  // 草
  const tuft = makeGrassTuftSource(scene);
  const flower = makeFlowerSource(scene);
  const pebble = makePebbleSource(scene);
  const mats: { m: Mesh; arr: number[] }[] = [
    { m: tuft, arr: [] },
    { m: flower, arr: [] },
    { m: pebble, arr: [] },
  ];
  ...
  let placed = 0;
  for (let i = 0; i < 4200 && placed < 720; i++) {
    const x = (vnoise(i * 3.1, 7) - 0.5) * 120;
    const z = (vnoise(11, i * 2.7) - 0.5) * 120;
    const h = terrainHeight(x, z);
    if (h < 0.62 || h > 3.6) continue;
    if (pathDist(x, z) < 1.9) continue;
    if (Math.hypot(x, z + 1) < 10.5) continue; // 広場
    if (Math.hypot(x - POND.x, z - POND.z) < POND.r + 1.5) continue;
    let nearBuilding = false;
    for (const b of BUILDINGS) {
      const p = POIS[b.id];
      if (Math.hypot(x - p.x, z - p.z) < Math.max(b.w, b.d) * 0.75) nearBuilding = true;
    }
    if (nearBuilding) continue;
    const r = vnoise(i * 1.7, 23);
    const bucket = r < 0.68 ? 0 : r < 0.84 ? 1 : 2;
    const s = 0.7 + vnoise(i, 31) * 0.7;
    Quaternion.RotationYawPitchRollToRef(vnoise(i, 41) * Math.PI * 2, 0, 0, q);
    Matrix.ComposeToRef(new Vector3(s, s * (0.8 + vnoise(i, 43) * 0.5), s), q, new Vector3(x, h - 0.02, z), mtx);
    mtx.copyToArray(mats[bucket].arr, mats[bucket].arr.length);
    placed++;
  }
  for (const { m, arr } of mats) {
    if (arr.length) m.thinInstanceSetBuffer('matrix', new Float32Array(arr), 16, true);
    else m.setEnabled(false);
  }
}

// 草・花は薄板なので両面ライティングの専用マテリアルを使う
...
function makeGrassTuftSource(scene: Scene): Mesh {
  const A = A0();
  const g1 = Color3.FromHexString('#79a562');
  for (let i = 0; i < 3; i++) {
    ...
    const c = jitterColor(g1, i + 1, 0.15);
    for (let k = 0; k < 4; k++) A.col.push(c.r * (k < 2 ? 0.78 : 1.05), c.g * (k < 2 ? 0.8 : 1.05), c.b * (k < 2 ? 0.78 : 1.05), 1);
...
function makeFlowerSource(scene: Scene): Mesh {
  const A = A0();
  ...
  A.idx.push(base, base + 1, base + 2, base + 1, base + 3, base + 2);
  appendBlob(A, 0, 0.38, 0, 0.07, 0.05, 0.07, Color3.FromHexString('#e8d9a0'), { segs: 5, noise: 0.06, bottomDark: 0.1 });
  const mesh = toMesh(scene, 'flowerSrc', A);
  mesh.material = getDecoMat(scene);
  return mesh;
}
