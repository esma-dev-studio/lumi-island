// v29 担当エージェント専用の Playwright 設定(並行作業のための切りわけ)。
//
//   - baseURL は 5225(自分の dev サーバーだけを見る)
//   - outputDir も 分ける(失敗の証拠が ほかのエージェントの走行と まざらない)
//   - webServer は 立てない。5225 は 自分で `npx vite --config vite.life.config.ts` で
//     上げてから走らせる(reuseExistingServer で 他人のサーバーを つかまない)
//
//   npx playwright test --config=playwright.life.config.ts
...
export default defineConfig({
  ...
  fullyParallel: false,
  workers: 1,
  outputDir: '.logs/pw-life-results',
  use: {
